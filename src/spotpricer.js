...
const spArgs = parse(Deno.args, {
    default: { host: undefined, port: undefined, topic: "homeassistant/sensor/", entity:"spotprice", area: "SE2", currency: "SEK" }
});
...
await publishDevice("Lowest upcomping spot price today", baseEntity + "_today_min", extremesToday.minVal/1000);
await publishDevice("Lowest upcomping spot price today time", baseEntity + "_today_min_time", extremesToday.minTime.toISOString());
await publishDevice("Highest upcomping spot price tomorrow", baseEntity + "_tomorrow_max", extremesTomorrow.maxVal/1000);
...
await publishDevice("Lowest upcomping spot price tomorrow", baseEntity + "_tomorrow_min", extremesTomorrow.minVal/1000);
