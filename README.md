...
With the default/example settings, the image will fetch spot prices every full hour, and forward it to homeassistant/sensor/spotprice__/state, it will also send homeassistant/sensor/spotprice__/config to make mqtt autodiscover the entities correctly.
